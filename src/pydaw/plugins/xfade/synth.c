...
PYFX_Descriptor *xfade_PYFX_descriptor(int index);
...
{
    t_xfade *plugin_data;
    lmalloc((void**)&plugin_data, sizeof(t_xfade));
    lmalloc((void**)&plugin_data->buffers, sizeof(float*) * 2);
    lmalloc((void**)&plugin_data->sc_buffers, sizeof(float*) * 2);
    ...
    t_pydaw_seq_event *ext_events, int ext_event_count)
{
    ...
    {
        v_xfade_process_midi_event(plugin_data, &events[event_pos]);
        ++event_pos;
    }
    ...
    {
        v_xfade_process_midi_event(plugin_data, &ext_events[f_i]);
        ++f_i;
    }
}
...
{
    t_xfade *plugin_data = (t_xfade*)instance;
    v_xfade_process_midi(instance, events, event_count,
